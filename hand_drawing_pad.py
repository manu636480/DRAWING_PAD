...
def check_thickness():
    global dct,text
    if dct["parameters"] in ['line','erase','marker'] and dct["thickness"] in [-1,0]:
        dct["thickness"] = 1
...
def track(event,x,y):
    ...
    if b or tab:
        if y1<100:
            ...
            if points:
                cv2.line(img, points[-1], (x, y), dct['color'], dct['thickness'])
            points.append((x, y))
          ...
          cv2.rectangle(img, (x - dct['thickness'], y - dct['thickness']), (x + dct['thickness'], y + dct['thickness']), background_color, -1)
        tab = 0
    if y1>100:
...
img_grid = img.copy()
prev_img = img.copy()
undo_stack.append(prev_img.copy())
redo_stack.clear()  # Clear redo stack on new action
    ...
    try:
        ...
        if hands:
            ...
            if (prev_lenght >= min_length and length < min_length):
                event=1
                a=1
        # Save state when gesture starts
                undo_stack.append(img.copy())
                redo_stack.clear()
            elif (prev_lenght < min_length and length >= min_length):
                event=4
